...
`npm run ledger:check` runs `node scripts/ledger.mjs check`, so a malformed entry fails when run (this repo has no vitest/jest; wire it into your normal pre-PR checklist, or add it as a CI step if you add one).
...
way a single append-only ledger file would.
